...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] pb-40 overflow-hidden relative">
          ...
          viewMode === 'grid' ? (
            <div className="space-y-32">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-24">
                {(isLoggedIn ? events : events.slice(0, 5)).map((event) => (
              ...
              {!isLoggedIn && events.length > 5 && (
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  className="bg-emerald-950 rounded-[4rem] p-12 md:p-20 text-center border border-gold-500/20 shadow-2xl relative overflow-hidden"
                >
                  <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none">
                    <Sparkles className="w-full h-full text-gold-500" />
                  </div>
                  <div className="relative z-10 space-y-8">
                    <h3 className="text-4xl md:text-5xl font-display text-white italic">Unlock the Full <span className="text-gold-500">Legend.</span></h3>
                    <p className="text-white/60 max-w-xl mx-auto font-medium text-lg">
                      There are {events.length - 5} more sacred festivals and cultural gatherings waiting for you. Join our exclusive community to see the full heritage calendar.
                    </p>
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-4">
                      <Link to="/register" className="px-10 py-5 bg-gold-500 text-emerald-950 rounded-2xl font-black text-xs tracking-widest uppercase hover:scale-105 transition-all shadow-xl">Join the Tribe</Link>
                      <Link to="/login" className="px-10 py-5 border-2 border-white/20 text-white rounded-2xl font-black text-xs tracking-widest uppercase hover:bg-white/10 transition-all">Member Login</Link>
                    </div>
                  </div>
                </motion.div>
              )}
      ...
      {/* 7. PARTNER WITH US (Eco-system Building) */}
